		...
		while (running) {
			if (launchtime + timeframe > System.currentTimeMillis()
				...
				try {
					...
					if(eventIndex < 0) {
						throw new SchedulerException("Event '" + event + "' is not handled by table '" + table.name + "'"); 
					}
					if(stateIndex < 0) {
						throw new SchedulerException("State with hash 0x" + Integer.toHexString(channel.getState()) + " is not handled by table '" + table.name + "'");
					}
